/*
              ...
              jpg image change monitor - 25Aug22 - created with https://processing.org/
              ...
              A log file (log.txt) is created when app closed and cleared when app opened.
                        ...
                        from: https://github.com/alanesq/imageChangeMonitor
                                                                      alanesq@disroot.org
*/
  ...
  // set up the cameras from json file if it exists 
    try {
      ...
      for (int i=0; i < camValues.size(); i++) {
        ...
        cameras.add(new camera(camData.getString("name"), camData.getString("url"), camData.getInt("enabled"), camData.getInt("maskx"), camData.getInt("masky"), camData.getInt("maskw"), camData.getInt("maskh")));
      } 
    } catch (Exception e) { 
      ...
      // create camera objects without using JSON file here (name, URL to the jpg, image detection maskx, masky, maskw, maskh)
        cameras.add(new camera("Front", "http://ha1:84/intranet/front.jpg", 1));                                 // url to access a jpg image
        //cameras.add(new camera("Back", "http://192.168.1.23/cam.jpg", 1));                                     // second camera 
        //cameras.add(new camera("ESP32", "http://192.168.2.19/jpg", 1));                                        // esp32cam using https://github.com/alanesq/esp32cam-demo
        //cameras.add(new camera("IPcam", "http://192.168.2.123/snapshot.cgi?user=guest&pwd=yourpassword", 1));  // using a commercial ip camera  
    }
...
void compareImages() {
    // compare old and new camera images using OpenCV and display images
      ...
      for (int i = 0; i < cameras.size(); i++) {                                     // step through all cameras
        ...
        if (cam.enabled != 1) continue;                                              // skip camera if not enabled
      ...
      // toggle cameras enabled flag
          if (int(key) >= 49 && int(key) <= 57) {    // if it is a number key (1 to 9)
            ...
            if (camNum < cameras.size()) {           // if this is a valid camera  
              ...
              if (cam.enabled == 0) cam.enabled = 1; else cam.enabled = 0;
...
public class camera{
  ...
  public int  enabled;                        // if this camera is enabled (0 or 1)
  ...
  public camera(String camName, String camURL, int camEnabled, int maskX, int maskY, int maskW, int maskH) {  
    if (camEnabled != 0 && camEnabled != 1) camEnabled = 1; 
    iloc = camURL; 
    cameraName = camName;
  ...
  public camera(String camName, String camURL, int camEnabled) {  
    this(camName, camURL, camEnabled, -1, 0, 0, 0);  
  }   // camera
  
  // refresh the camera image
  public boolean update() {       
    if (enabled == 0) return false;   // abort if camera is disabled
...
public int motion() {  
  ...
  if (enabled == 0) return 0;      // if camera is disabled exit reporting no movement
